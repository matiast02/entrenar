...
@section('contenido')
    ...
    <form id="crear-deporte" class="form-horizontal col-md-6"  >
        <div class="panel panel-flat">
            <div class="panel-heading">
                <h5 class="panel-title">Nuevo Deporte<a class="heading-elements-toggle"><i class="icon-more"></i></a></h5>
...
@section('scripts')
    <script type="text/javascript">
        ...
        //enviar formulario
        $("#crear-deporte").submit(function(e) {

            $.ajax({
                type: 'post',
                url: "{{route('deportes.guardar')}}",
                data:  $("#crear-deporte").serialize(),
                dataType: 'json',
                success: function(data){
                    swal("Bien!", "Los datos se guardaron correctamente.", "success");
                    //resetear estilos
                    $('.form-group').removeClass("has-error has-feedback");
                    $('#crear-deporte').find('.form-control-feedback').html('');
                    $('#crear-deporte').find('.help-block').html('');
                    $('#crear-deporte')[0].reset();
                },
                error: function(data){
                    //resetear estilos
                    $('.form-group').removeClass("has-error has-feedback");
