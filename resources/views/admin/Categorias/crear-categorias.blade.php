...
@section('contenido')
    ...
    <form id="crear-categoria" class="form-horizontal col-md-6"  >
        <div class="panel panel-flat">
            <div class="panel-heading">
                <h5 class="panel-title">Nueva Categoria<a class="heading-elements-toggle"><i class="icon-more"></i></a></h5>
...
@section('scripts')
    <script type="text/javascript">
        ...
        //enviar formulario
        $("#crear-categoria").submit(function(e) {

            $.ajax({
                type: 'post',
                url: "{{route('categorias.guardar')}}",
                data:  $("#crear-categoria").serialize(),
                dataType: 'json',
                success: function(data){
                    swal("Bien!", "Los datos se guardaron correctamente.", "success");
                    //resetear estilos
                    $('.form-group').removeClass("has-error has-feedback");
                    $('#crear-categoria').find('.form-control-feedback').html('');
                    $('#crear-categoria').find('.help-block').html('');
                    $('#crear-categoria')[0].reset();
                },
                error: function(data){
                    //resetear estilos
                    $('.form-group').removeClass("has-error has-feedback");
